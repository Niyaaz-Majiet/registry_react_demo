const DEFAULT_CAMPAIGNS= [{"id":1,"name":"Divavu","startDate":"9/19/2017","endDate":"3/9/2018","Budget":88377},
...
const setLocalStorageObject = (path :string,dataObject:object) => {
    let jsonString = JSON.stringify(dataObject)
    localStorage.setItem(path, jsonString);
}

const getLocalStorageObject = (path:string) => {
    let jsonObject = JSON.parse(localStorage.getItem(path) as string);
    return jsonObject;
}

const removeStorageObjectByPath = (path:string) => {
...
export {setLocalStorageObject,getLocalStorageObject,removeStorageObjects,removeStorageObjectByPath,DEFAULT_CAMPAIGNS,CAMPAIGN};
